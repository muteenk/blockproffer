...
contract PollSystem {
   ...
   */
  function createPoll(string memory _title, string memory _description, uint _dateCreated, uint _endDate) public 
  {
    numberOfPolls++;
   ...
   */
  function createPollCandidate(string memory _candidate, uint pollIndex) public 
  {
   ...
   */
  function getNumberOfVotes(uint pollIndex, uint candidateIndex) external pure returns (uint)
  {
   ...
   * @params: pollIndex (the id of the poll), CandidateIndex (id of the candidate)
   * @return: string memory (contains the name of a candidate)
   */
  function getCandidateName(uint pollIndex, uint candidateIndex) external pure returns (string memory)
  {
   ...
   */
  function getNumberOfCandidates(uint pollIndex) external pure returns (uint) 
  {
   ...
   */
  function checkIfOwner(uint pollIndex) external pure returns (uint) 
  {
   ...
   * @return: uint (contains the number of candidates)
   */
  function getPollDetails(uint pollIndex) external pure returns (string memory _title, string memory _description, uint _dateCreated, uint _endDate) 
  {
