...
const createPoll = async (roomID) => {
  try {
    if (!ethereum) return alert('Please install Metamask')
    const connectedAccount = getGlobalState('connectedAccount')
    const contract = getEtheriumContract()
    tx = await contract.createPoll(roomID)
    await tx.wait()
    // await getPolls()
  } catch (error) {
    reportError(error)
...
const viewResult = async (roomID) => {
  try {
    if (!ethereum) return alert('Please install Metamask')
    const connectedAccount = getGlobalState('connectedAccount')
    const contract = getEtheriumContract()
    tx = await contract.viewResult(roomID)
    await tx.wait()
    // await getPolls()
  } catch (error) {
    reportError(error)
...
const vote = async (roomID, option) => {
  try {
    if (!ethereum) return alert('Please install Metamask')
    const connectedAccount = getGlobalState('connectedAccount')
    const contract = getEtheriumContract()
    tx = await contract.vote(roomID, option)
    await tx.wait()
    // await getPolls()
  } catch (error) {
    reportError(error)
